...
Carrega os arquivos **nesta ordem** (atenção: `image-slot.js` e `tweaks-panel.jsx` ficam na **raiz**, não em `js/`):
...
| Arquivo | Papel |
|---|---|
| `Proposta Solax.html` | Ponto de entrada (dev). |
| `Proposta Solax (standalone).html` | Versão self-contained (tudo embutido) — abre offline, só pra "ver rodando". Regenerar após mudanças. |
...
| `image-slot.js` | Web component `<image-slot>` (drag-and-drop persistente). **Raiz.** |
| `tweaks-panel.jsx` | Painel de tweaks (ferramenta de design). **Raiz.** |
...
| ID do slot | Descrição | Default em `assets/` |
|---|---|---|
| `fabricante-logo` | Logo Solax | `logo-solax-power-branco-crop.png` |
...
> Imagens "fixas" do fabricante são defaults assados no arquivo. Para os outros 3 fabricantes, troca-se **apenas** este grupo + os nomes em `battery_manufacturer` / `inverter_manufacturer`. O miolo institucional + comercial é compartilhado.
